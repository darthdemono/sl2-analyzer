...
| Sekiro: Shadows Die Twice | `S0000.sl2` | Yes | **full** | play time, journey, Sen, Attack Power, max HP and Posture, every item carried and stored, bosses from Memories |
...
| `db_sdt/boss_flags.json` | `{boss: flag_id}` | 15 | Boss-defeat event flag IDs. Shipped for the **names**, since the flags themselves are not read, because where Sekiro's flag region sits in the save has never been published. See below |
