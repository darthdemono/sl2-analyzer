...
A tier says how deep the read goes. It says nothing about how much evidence sits behind it, and those are not the same thing. So here is the corpus. **260 real `.sl2` files, 442 characters, every one of which parses today**, and this is how they are distributed.
...
| Elden Ring | 19 | 182 | **medium for what it prints, nothing for progress** | Two personal saves plus [Bergbok's build collection](https://github.com/Bergbok/Elden-Ring-Saves), 17 files of ten characters each. Every one of the 182 resolves a stat block that agrees with the independently stored roster level, and three were checked attribute by attribute against the in-game Status screens the collection ships as its own documentation — level, HP, stamina, runes held and all eight attributes exact. What is still missing is not confidence, it is coverage: no ladder, no differential pair, so the flag region is unsolved and no boss flag, no grace and no pickup is read at all. |

The Elden Ring row is what a corpus is *for*, so it is worth saying what widening it did. Going from 12 characters to 182 immediately broke three things that 12 characters could never have shown. The stat block is **not 4-byte aligned** — variable-length data sits in front of it, the character name among it — and the old search stepped 4 bytes at a time, so it walked over three quarters of all characters and found only 29 of 182. Worse, on five slots the level identity matched a coincidental run of bytes megabytes past the real block, and the tool printed levels of 2, 13, 22 and 25 for characters the game puts at 125, 150 and 160. Both are fixed: the search is anchored on the level field and every candidate is cross-checked against the roster, which is stored separately and was right all along. All 182 now read, and the five wrong numbers are gone.

The third was in the item walk, and it is worth separating what it broke from what it did not. The GaItem array starts at 0x30 and the walk began at 0x20, which matters because the entries are variable-length and each one's length is taken from the id just read — so a wrong start picks its stride from garbage and never recovers. Across the same 182 characters, 148 of them read category codes that do not exist; from 0x30, none do. But the item *names* were never wrong: garbage ids match nothing in the tables and were dropped, and the resolved list is identical either way, on a 25-item character as much as a 1,100-item one. What was wrong is the count in the "some IDs were not in the name database" line, which the garbage inflated — one character claimed 1,001 unrecognised items and actually has 74. A save set that only ever agreed with itself would have hidden all three faults indefinitely.
...
<p align="center">
  <img src="graphics/shots/combined-journey.webp" width="900"
       alt="The journey chart: one box per character across several runs and several games, in the order the files were written" />
</p>
...
**The Healing Gourd took a differential AND a conservation check, because the differential alone would have got the meaning wrong.** A four-minute pair either side of handing Emma one Gourd Seed moved exactly one byte in the player struct, `0x34562`, from 7 to 8 — the other five words that moved are play time, two timers and a pair that only tracks with play time. That much says the byte counts *something* about seeds. The obvious reading, "seeds used, 1-based like the other two", is wrong, and what catches it is the item list: the Healing Gourd is an ordinary inventory row whose quantity is its remaining charges, and across 45 saves that quantity never once exceeds this byte and repeatedly equals it — 4 on all eight saves reading 4, 5 at 5, 7 at 7. If the byte were a seed tally with charges some fixed amount above it, a gourd refilled at every idol would have to read higher somewhere. It never does. So the byte IS the charge count, a fresh gourd holds one, and one seed buys one — which still makes `gourd - 1` the seeds consumed, but for a checkable reason rather than an assumed base. The ceiling is the game's own `maxNum` for the item, 10, read out of the install rather than a wiki.

The second check is worth naming on its own, because it is stronger than any single pair. A Gourd Seed is destroyed on use, so **the byte plus the seeds still in the inventory can never fall**. Across the whole 45-save ladder it never does, and on every save where a held seed disappears the byte takes exactly the +1 the inventory lost. A wrong offset cannot stay conserved against a list parsed independently of it.
...
That matters more in Sekiro than it would elsewhere, because the arithmetic can only count how many Memories were spent, and a spent Memory is gone from the inventory. A held one does name its boss (`Memory: Gyoubu Oniwa`, filed with the key items), and it counts as `soul held` evidence. The flag names the boss either way. **Two goods rows called plain "Memory" are not Memories.** Across the whole 60-save ladder, row `5100`'s quantity is the number of Memories consumed and row `5400` is 1 exactly while a boss Memory is carried unspent. Both are the game's own counters. Reading them as tokens once made a character with nothing to spend report a Memory "still held" and list `Memory ×5` in its inventory, so they are now counted with the engine-state rows and never printed.
...
| `db_sdt/item_flags.json` | `{family: [[id, name, area]]}` | 2 families / 927 flags | Sekiro item-lot flags, `50000000 + lot id`. The 826 world pickups **are read** — the family sits in a second bank of flag categories, 66 along from each map's own — for the nine areas an idol names, which is 581 of them. The 101 shop-purchase rows are not read |
| `db_sdt/lot_flags.json` | `{moved: {id: flag}, misfiled: [[id, name]]}` | 206 moved / 1 misfiled | Corrections to the table above from the game's own `ItemLotParam`, generated by `tools/gen_sdt_lot_flags.py`. `50000000 + lot id` is a shortcut, not the rule: every prosthetic part, esoteric text, Gourd Seed and world Prayer Bead sets a **global** flag instead (`6500` for the Shuriken Wheel), so without this they read as never picked up |
...
| Sekiro | Boss defeats | 15 | of 18 tracked | `Boss.cs`. The only thing that can name a Sekiro boss whose Memory has already been consumed |
...
| Sekiro | World item pickups | 581 in 9 areas | of the 826 lots the table knows | The item-lot family (`5xxxxxxx`) sits in a SECOND bank of flag categories, 66 along from each map's own — the same offset Dark Souls III uses, found here from the other end. Derived from a 25-second save pair whose only pickup was one Mibu Balloon of Wealth: of the eleven rows carrying that name, nine are excluded by the bit index alone and only one puts its block on the region's `0x500` grid. The 237 rows in families no idol names have no category to read, and six more are `6xxxxxxx` — a top-level group with no seat of its own, which used to fall through to the map branch and alias onto a real flag: `61300000`, a Prayer Bead, landed byte-for-byte on the Underground Waterway idol's bit and reported itself collected the moment that idol was lit. Those are refused now. **The id itself was a shortcut too:** a row is filed under `50000000 + lot id`, but the game's `ItemLotParam` gives every lot its own flag, and a hundred of them differ. Every prosthetic part, esoteric text, Gourd Seed and world Prayer Bead sets a global flag, so those read as never picked up on a save that had picked up every one. `db_sdt/lot_flags.json` reads each at its real flag, and the ladder checks it: each first sets in the save that picked the item up, and the Prayer Bead flags count the beads earned (four per necklace plus the ones held) on all 60 saves, one short only once a shop bead has been bought. Two rows drop out: one misfiled under another lot's id, and one whose real flag is in a family with no seat. So the denominator says 581, and the section says why |
...
graphics/shots/      screenshots used by the page and this file, generated by
                     scratch/shots_site.mjs against real saves
