...
def ds1_attach_flags(ch, buf, base_dir, game):
    ...
    # World state: the bells, the Lordvessel, shortcut doors and levers, fog gates,
    # NPCs and the covenant joined. Same region, same guards — they cost one more table
    # lookup each and are the largest single thing DS1 was leaving on the floor.
    world = []
    for cat, rows in load_ds1_known_flags(base_dir).items():
        got, missing = [], []
        for off, mask, name in rows:
            v = u32(buf, base + off)
            (got if v is not None and v & mask else missing).append(name)
        world.append((cat, len(got), got, len(rows), missing))
    if any(c for _a, c, _n, _t, _m in world):
        ch["world_flags"] = world
    # World EVENTS: the same region again, from a much larger table extracted out of the
    # committed event scripts. Counts only — the names are FromSoft's own Japanese and are
    # not translated here. Two of its families are known not to mean what they say (see
    # tools/gen_ds1_world_events.py), which is why this is a separate section from the
    # named world state above rather than more rows inside it.
    events = []
    for cat, rows in load_ds1_world_events(base_dir).items():
        got = sum(1 for off, mask, _n in rows if (u32(buf, base + off) or 0) & mask)
        events.append((cat, got, len(rows)))
    if any(c for _a, c, _t in events):
        ch["world_events"] = events


##
# @brief Load the DS1 world-state flag table (category → [[offset, mask, name]]).
# @details Generated by `tools/gen_ds1_known_flags.py`, whose id→(offset, mask) function
# reproduces all twelve hand-checked entries of `boss_flags.json` exactly — which is why
# these 44 are trusted without each being verified on its own. Cached per dir.
_KNOWN_FLAG_CACHE = {}


def load_ds1_known_flags(base_dir):
    if base_dir not in _KNOWN_FLAG_CACHE:
        path = os.path.join(base_dir, "db_ds1", "known_flags.json")
        try:
            with open(path, encoding="utf-8") as f:
                _KNOWN_FLAG_CACHE[base_dir] = json.load(f)
        except (OSError, ValueError):
            _KNOWN_FLAG_CACHE[base_dir] = {}
    return _KNOWN_FLAG_CACHE[base_dir]


##
# @brief Load the DS1 world-event table (family → [[offset, mask, Japanese name]]).
# @details Generated by `tools/gen_ds1_world_events.py` from the committed `.emevd` plus
# FromSoft's own `.emeld` event names. Every row is `derived` and none is verified against
# a save here — it decodes and it discriminates, and that is all that is claimed. Read the
# generator before believing a family name: `Levers` reads 0 on a finished run and
# `Boss-fight flags` counts flags rather than bosses. Cached per dir.
_WORLD_EVENT_CACHE = {}


def load_ds1_world_events(base_dir):
    if base_dir not in _WORLD_EVENT_CACHE:
        path = os.path.join(base_dir, "db_ds1", "world_events.json")
        try:
            with open(path, encoding="utf-8") as f:
                _WORLD_EVENT_CACHE[base_dir] = json.load(f)
        except (OSError, ValueError):
            _WORLD_EVENT_CACHE[base_dir] = {}
    return _WORLD_EVENT_CACHE[base_dir]
