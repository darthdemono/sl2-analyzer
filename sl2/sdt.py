...
# @details The entity id is normally the defeat flag in Sekiro. Four map-state encounters
# lose that local flag after their map changes, so @ref load_sdt_miniboss_fallbacks supplies
# a permanent companion or ItemLot reward flag instead. A companion can name the key item
# that gates the alternate map state, preventing a shared local flag from false-matching.
# The roster itself remains generated by `tools/gamefiles.py roster --write` from the
# health-bar events.
_MINIBOSS_CACHE = {}
...
## @brief Persistent reward flags for minibosses whose entity flags reset with map state.
_MINIBOSS_FALLBACK_CACHE = {}


def load_sdt_miniboss_fallbacks(base_dir):
    if base_dir not in _MINIBOSS_FALLBACK_CACHE:
        path = os.path.join(base_dir, "db_sdt", "miniboss_fallbacks.json")
        try:
            with open(path, encoding="utf-8") as f:
                _MINIBOSS_FALLBACK_CACHE[base_dir] = json.load(f)
        except (OSError, ValueError):
            _MINIBOSS_FALLBACK_CACHE[base_dir] = {}
    return _MINIBOSS_FALLBACK_CACHE[base_dir]
...
def sdt_attach_flags(ch, buf, base_dir):
    ...
    fallbacks = load_sdt_miniboss_fallbacks(base_dir)
    keys = {name for name, _qty in ch.get("key_items") or []}
    for area, enemies in load_sdt_minibosses(base_dir).items():
        dead, alive = [], []
        for eid, name in enemies:
            fallback = fallbacks.get(str(eid))
            if isinstance(fallback, dict):
                gate = fallback.get("key_item")
                fallback = fallback.get("flag") if not gate or gate in keys else None
            killed = sdt_flag(buf, int(eid)) or (
                fallback is not None and sdt_flag(buf, int(fallback))
            )
            (dead if killed else alive).append(name)
