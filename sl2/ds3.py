...
## @brief Load the one-time-enemy table (db_ds3/enemies.json): area → list of
#  @c [distance, bit, enemy type] — one per enemy that does not respawn and whose death
#  DS3 therefore has to remember. Mimics, Crystal Lizards, Black Knights, the Boreal
#  Outrider Knights. Extracted from the committed `.emevd`, where each is a call to a
#  `common_func` template taking a death flag and an entity id, so these are DEATH flags
#  rather than the drop flags the same enemies' loot already sets. Trusted because the
#  count climbs 3 → 98 with no regressions across a 79-save ladder and the per-area
#  breakdown tracks where the character actually went; see `tools/gen_ds3_enemies.py`.
#  Unlike the per-map boss flags these survive a new journey. Cached. Returns {} if absent.
_DS3_ENEMY_CACHE = {}


def load_ds3_enemies(base_dir):
    if base_dir not in _DS3_ENEMY_CACHE:
        path = os.path.join(base_dir, "db_ds3", "enemies.json")
        try:
            with open(path, encoding="utf-8") as f:
                _DS3_ENEMY_CACHE[base_dir] = json.load(f)
        except (OSError, ValueError):
            _DS3_ENEMY_CACHE[base_dir] = {}
    return _DS3_ENEMY_CACHE[base_dir]


## @brief Load the DS3 NPC-state table (db_ds3/npcs.json): family → [[distance, bit,
#  label]] — NPCs killed, NPCs turned hostile, questline states. Generated by
#  `tools/gen_ds3_npcs.py`, on the common-group base `111 + 128*g` derived there and
#  confirmed by a dated kill on the ladder. Two caveats live in that generator and are
#  repeated in the section note: an English description can sit on the wrong flag (one
#  demonstrably does), and a row with no description prints an entity id because inventing
#  a name is worse. Cached. Returns {} if absent.
_DS3_NPC_CACHE = {}


def load_ds3_npcs(base_dir):
    if base_dir not in _DS3_NPC_CACHE:
        path = os.path.join(base_dir, "db_ds3", "npcs.json")
        try:
            with open(path, encoding="utf-8") as f:
                _DS3_NPC_CACHE[base_dir] = json.load(f)
        except (OSError, ValueError):
            _DS3_NPC_CACHE[base_dir] = {}
    return _DS3_NPC_CACHE[base_dir]
...
def ds3_attach_flags(ch, buf, base, base_dir):
    ...
    foes, any_dead = [], False
    for area, enemies in load_ds3_enemies(base_dir).items():
        dead, alive = [], []
        for dist, bit, name in enemies:
            val = u8(buf, base + dist)
            (dead if val is not None and val & (1 << bit) else alive).append(name)
        any_dead = any_dead or bool(dead)
        foes.append((area, len(dead), dead, len(enemies), alive))
    if any_dead:
        ch["enemies"] = foes
    # NPC states: killed, turned hostile, questline milestones. Counts and the names of
    # what fired — the ones that have NOT fired are deliberately not listed, because half
    # of them are mutually exclusive outcomes of one questline and a "missing" list would
    # read as a to-do list of things a player cannot all do.
    npcs = []
    for family, marks in load_ds3_npcs(base_dir).items():
        got = []
        for dist, bit, label in marks:
            val = u8(buf, base + dist)
            if val is not None and val & (1 << bit):
                got.append(label)
        npcs.append((family, len(got), got, len(marks)))
    if any(c for _f, c, _g, _t in npcs):
        ch["npc_states"] = npcs
