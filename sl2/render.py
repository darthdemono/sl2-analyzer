...
STAT_CAPS = {
    "ds1": OrderedDict(
        [
            (
                "Vitality",
                "soft caps 30 (~1,100 HP) & 50 (~1,500 HP), rising to ~1,900 at 99",
            ),
            (
                "Attunement",
                "1 slot at 10, then 12/14/16/19/23/28/34/41/50 — 10 slots max at 50",
            ),
            (
                "Endurance",
                "stamina maxes at 40 (160); equip load keeps rising (~+1/lvl) to 99",
            ),
    ...
    "ds2sotfs": OrderedDict(
        [
            ...
            (
                "Vitality",
                "soft caps 29/49/70; +1.5 load/lvl to 29, +1 to 49, +0.5 to 69, +0.25 after",
            ),
            (
                "Attunement",
                "slots at 10/13/16/20/25/30/40/50/60/75/94; cast-speed breakpoints 30/45/60/80",
            ),
    ...
    "ds3": OrderedDict(
        [
            ...
            (
                "Attunement",
                "FP soft cap 35 (450 max at 99); slots at 10/14/18/24/30/40/50/60/80/99",
            ),
...
def guess_build(stats):
    ...
    phys, cast = (
        g("Strength") + g("Dexterity"),
        g("Intelligence") + g("Faith") + g("Attunement"),
    )
    if cast > phys:
...
def memories_line(m):
    ...
    lap = (
        "across every journey so far — Attack Power carries into New Game+ while "
        "the Memories do not"
        if m["cumulative"]
        else "this journey; a boss that drops no Memory is not counted either way"
    )
    return (
        f"{total} Memory-dropping boss{'' if total == 1 else 'es'} defeated"
...
def vitality_necklaces(vitality):
    ...
    return (
        ...
        if used == 0
        else f"{used} Prayer Necklace{'' if used == 1 else 's'} used — four Prayer Beads "
...
def fmt(value):
    return (
        "—" if value is None else f"{value:,}" if isinstance(value, int) else str(value)
    )
...
SDT_BONFIRE_NOTE = (
    "each idol's own flag bit, by area — a floor, and one that starts "
...
DS1_WORLD_NOTE = (
    "one-off world events, each read from its own flag — exact, but only "
...
DS3_PICKUP_NOTE = (
    "one-off world items picked up, from each area's pickup flags — "
...
def lords_line(lords):
    ...
    if lords["placed"] is None:  # NG+: the thrones reset, the defeat flags do not
        return (
            f"{len(lords['named'])} of {lords['total']}{named}"
            "  _(NG+ — only the mapped throne flags are read, so this is a floor)_"
        )
    return (
        f"{lords['placed']} of {lords['total']}{named}"
...
def md_for_character(ch, slot_no):
    ...
    if ch["level"] is not None:
        L.append(
            f"- **{'Level' if ch['game'] == 'er' else 'Soul Level'}:** {ch['level']}"
        )
    if ch["klass"]:
    ...
    if ch["soul_memory"] is not None:
        L.append(
            f"- **Soul Memory:** {fmt(ch['soul_memory'])}  _(total souls earned — main progress metric)_"
        )
    if ch.get("play_time"):
    ...
    if ch.get("skill_points") is not None:
        L.append(
            f"- **Skill Points Held:** {ch['skill_points']}"
    ...
    if ch.get("embered") is not None:
        L.append(
            ...
            if ch["embered"]
            else "- **Embered:** No  _(hollow — Max HP above is the base value)_"
        )
    if ch.get("fp") is not None:
        L.append(f"- **Max FP:** {fmt(ch['fp'])}")
    if ch.get("hollow_lvl"):
        L.append(
            f"- **Hollowing:** {ch['hollow_lvl']}  _(higher = more deaths without an effigy)_"
        )
    ...
    if ch.get("endings"):
        ends = ch["endings"]
        L.append(
            f"- **Ending{'' if len(ends) == 1 else 's'} Reached:** " + " · ".join(ends)
        )
    ...
    if ch["stats"]:
        ...
        if rows:
            ...
            L += [
                "<details>",
                "<summary><b>Attribute Scaling</b> — what each stat "
        ...
        if ch["game"] in DS2_GAMES:
            ...
            agl = f"{d['agility']}" + (
                f"  _({d['iframes']} roll i-frames)_" if d["iframes"] else ""
            )
            L += [
                "### Derived Stats  _(computed from attributes — base values before "
        ...
        if ch["game"] == "ds3":
            ...
            L += [
                "### Derived Stats  _(computed from attributes, plus the documented "
            ...
            already = [
                f"+{v:g}% {'Max HP' if k == 'hp_pct' else 'Stamina'} ({n})"
                for k in ("hp_pct", "stam_pct")
                for n, v in w[k]
            ]
            if already:
                L.append(
                    f"- **Also from rings:** {', '.join(already)}  _(Max HP and "
    ...
    if ch.get("bonfires") and not ch.get("bonfire_areas"):
        L += [
            f"### Bonfires Discovered ({len(ch['bonfires'])})  _(areas reached — a "
            "floor on progress)_",
            "",
        ]
    ...
    if ch.get("bonfire_areas"):
        ...
        note = (
            DS1_BONFIRE_NOTE
            if ch.get("game") in ("dsr", "ptde")
            else DS2_BONFIRE_NOTE
            if ch.get("game") in DS2_GAMES
            else SDT_BONFIRE_NOTE
            if ch.get("game") == "sdt"
        ...
        head = (
            "Sculptor's Idols Lit" if ch.get("game") == "sdt" else "Bonfires Discovered"
        )
        L += [f"### {head} ({lit} of {total}, in {n} of {areas} areas)  _({note})_", ""]
    ...
    if ch.get("covenants"):
        ...
        L += [
            f"### Covenants Found ({count})  _(discovered — a floor; "
    ...
    if ch.get("pickups"):
        ...
        todo = [
            (area, missing) for area, c, _t, missing in ch["pickups"] if c and missing
        ]
        if todo:
            L += [
                "<details>",
    ...
    if ch.get("questlines"):
        ...
        L += [
            "### Rewards Obtained  _(one-off rewards from NPCs, invaders and "
            "landmark pickups — a progress floor)_",
            "",
        ]
        L += [
            f"- **{src}:** {', '.join(rw)}" for src, rw in ch["questlines"].items()
        ] + [""]
    if ch.get("bosses"):
        SRC = {
            "flag": "confirmed",
        ...
        L += [
            f"### Bosses Defeated ({count})  _(a floor — from defeat "
        ...
        if avail:
            L += [
                f"_Available now — every prerequisite dead and the area already "
                f"reached (from the game's fixed route, not this save): "
                f"{' · '.join(avail)}._",
                "",
            ]
        note = missing_note(
            "No evidence yet" + (", and behind something else" if avail else ""), rest
        )
        if note:
            L += [note, ""]

    if (
        ch.get("equipped_weapons")
        or ch.get("equipped_armor")
        or ch.get("equipped_rings")
        or ch.get("equipped_ammo")
    ):
        L += ["### Equipped  _(worn gear read from the equip slots)_", ""]
        L += [
            f"- **{slot}:** {name}"
            for slot, name in ch.get("equipped_weapons", {}).items()
        ]
        L += [
            f"- **{slot}:** {name}"
            for slot, name in ch.get("equipped_armor", {}).items()
        ]
    ...
    if ch["unknown_count"]:
        L += [
            f"_{ch['unknown_count']} inventory item(s) had IDs not in the name "
            "database (upgraded / infused variants) and were omitted._",
            "",
        ]
    if ch.get("internal_count"):
        L += [
            f"_{ch['internal_count']} further entr"
