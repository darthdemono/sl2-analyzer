...
function runTimeline(rows, refs) {
  ...
  const rewards = firstSeen(rows, (r) => Object.entries(r.questlines)
    .flatMap(([q, v]) => v.map((rw) => [q, rw])));
  if (rewards.length) {
  ...
  const est = rows.filter((r, i) => r.estus != null
    && (i === 0 || r.estus !== rows[i - 1].estus));
  if (est.length > 1) {
...
function runSection(key, rows, refs, charFor) {
  ...
  L.push("");
  const note = [forks ? `${forks} fork${forks === 1 ? "" : "s"}` : "No forks",
  ...
  if (restarts.size) {
    L.push("");
    L.push("_A box marked SEPARATE LINE could not descend from anything before it — it "
  ...
  if (ch) {
    ...
    // run's "##", not beside it.
    L.push(...mdCharacter(ch, last.slot).split("\n")
...
export function buildCombined(entries, meta = null, now = new Date()) {
  ...
  const L = ["# FromSoftware Saves — Combined Playthrough Timeline", "",
...
function combinedFooter(snaps, runs, meta, now) {
  ...
  const L = ["---", "", "<details>",
    ...
    "", `- **Save files read:** ${files.size}`, `- **Runs (characters):** ${runs.size}`,
    "", "**Games covered**", ""];
  for (const [, [title, tier]] of seen) L.push(`- **${title}:** support tier ${tier}`);
  if (meta && Object.keys(meta).length) {
    L.push("", "**Setup**  _(supplied by the caller — not read from the saves, which "
      + "cannot know any of it)_", "");
    for (const [key, value] of Object.entries(meta)) {
      const lab = META_LABEL[key] || (key.replace(/_/g, " ").charAt(0).toUpperCase()
        + key.replace(/_/g, " ").slice(1));
      L.push(`- **${lab}:** ${Array.isArray(value) ? value.join(" · ") : value}`);
    }
  }
  L.push("", "Everything above is read out of the saves themselves, in this browser or "
...
const META_LABEL = { dlc: "DLC", os: "OS", cpu: "CPU", gpu: "GPU", ram: "RAM",
  mangohud: "MangoHud", gamemode: "GameMode", dxvk: "DXVK", fps: "FPS", hdr: "HDR",
  url: "URL", id: "ID" };
