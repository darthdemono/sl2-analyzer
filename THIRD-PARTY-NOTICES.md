...
| [alfizari/Elden-Ring-Nightreign-Save-Editor](https://github.com/alfizari/Elden-Ring-Nightreign-Save-Editor) | Elden Ring Nightreign's save key and container layout — the AES-128 key, the IV-at-the-front entry shape, and where the account sits in the menu block. The key is filed there under the name `DS2_KEY`, which it is not; Dark Souls II's is a different sixteen bytes and is credited above. Nothing was taken on trust: every Nightreign entry carries an MD5 of its own plaintext, and the key is accepted here because all fourteen entries of the test save hash to what they carry. No code from it is reproduced. |
